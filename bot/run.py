...
commands = [
    ...
    types.BotCommand(command="chat_link", description="Create chat link about publication"),
...
@dp.message(Command("chat_link"))
async def chat_link_command_handler(message: types.Message):
    pdf_id = "3fa85f64-5717-4562-b3fc-2c963f66afa6" #message.get_args()
    userid = message.from_user.id
    ...
    chat_link = await create_chat_link(userid, pdf_id)

    await message.answer(f"Посилання на обговорення публікації: {chat_link}")



async def create_chat_link(userid, pdf_id: str) -> str:
    api_url = "https://api.inchat.pp.ua:4433/chat/link"
    ...
    async with aiohttp.ClientSession() as client:
        ...
        payload = {
            "pdf_id": pdf_id,
            "text": "Відповідай українською мовою."
        }
        payload_as_text = json.dumps(payload)

        response = await client.post(api_url, data=payload_as_text, headers=headers)
        answer_json = await response.json()
        if "link_tg" in answer_json.keys():
            chat_link = answer_json["link_tg"]
            logging.info(f"[*]	Chat Link {userid}: {chat_link}")
            user_credentials[userid]["chat_link"] = chat_link
            return chat_link
        else:
            err_json = await response.json()
            logging.info(f"Error Response for {userid}: {err_json}")
            raise ValueError(f"Не вдалося отримати  chat link. HTTP-код: {err_json}")
